...
// Hardcoded starting balances from backup.json (same as restore_to_payroll.js)
...
// Hardcoded historical log IDs present in backup.json (same as restore_to_payroll.js)
...
async function main() {
  ...
  await db.asyncLocalStorage.run({ dbName: targetDb }, async () => {
    console.log(`\n==================================================`);
    console.log(`🧼 [Clean Reset] Target Database: ${targetDb}`);
    console.log(`==================================================\n`);
    
    const conn = await db.getConnection();

    try {
      const [students] = await conn.query('SELECT id, name, number, calory FROM students ORDER BY number ASC');
      ...
      const corrections = [];
      console.log('--------------------------------------------------------------------------------');
      console.log('번호\t이름\t현재 잔액\t순수 활동 수입\t최종 정상 예상 잔액\t조정액');
      console.log('--------------------------------------------------------------------------------');
      ...
      for (const st of students) {
        ...
        // Filter out any stock-related logs entirely (buy/sell/leverage)
        const cleanNonStockLogs = newLogs.filter(log => {
          const summary = log.summary || '';
          return !(summary.includes('주식') || summary.includes('매수') || summary.includes('매도'));
        });

        let sumCleanDeltas = 0;
        for (const log of cleanNonStockLogs) {
      ...
      const toCorrect = corrections.filter(c => !c.protected);
      const protectedList = corrections.filter(c => c.protected);

      if (toCorrect.length === 0) {
        console.log('✨ 분석 완료: 조정이 필요한 비정상 현금 잔액이 감지되지 않았습니다.');
      } else {
        console.log(`⚠️ 총 ${toCorrect.length}명의 학생에게서 현금 잔액 오차가 감지되었습니다.`);
      }
      ...
      rl.question('\n⚠️ [최종 확인] 모든 학생의 주식(stock_portfolio)을 0주로 강제 초기화하고, 현금 잔액을 위의 순수 노동 잔액으로 완벽히 보정하시겠습니까? (y/n): ', async (answer) => {
        if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
          console.log('\n⚙️ 학급 경제 클린 리셋 및 잔액 보정 시작...');
          await conn.beginTransaction();
          try {
            // 1. Reset all stock portfolios in the students table to NULL/empty
            await conn.query('UPDATE students SET stock_portfolio = NULL');
            console.log('✅ [1단계] 전원 주식 보유량(0주) 완전 초기화 완료!');

            // 2. Delete all stock transactions from activity_logs to wipe out duplicate trade logs
            await conn.query("DELETE FROM activity_logs WHERE summary LIKE '%주식%' OR summary LIKE '%매수%' OR summary LIKE '%매도%'");
            console.log('✅ [2단계] 오염된 가짜 주식 거래 활동 로그 삭제 완료!');

            // 3. Update cash balances for students (except protected ones)
            let correctCount = 0;
            for (const corr of toCorrect) {
            ...
            // For protected students, we also reset their stock to NULL, but do we update their cash?
            // If they are protected, they keep their current database cash.
            for (const corr of protectedList) {
              console.log(`ℹ️ [#${corr.number} ${corr.name}] 잔액 보정 제외 (보호 대상): 현재 ${corr.currentCalory} Cal 유지`);
            }

            await conn.commit();
            console.log(`\n🎉 클린 리셋 완료! 총 ${correctCount}명 학생의 현금 잔액이 순수 노동 자산으로 완벽히 복원되었습니다!`);
          } catch (err) {
            await conn.rollback();
