...
async function main() {
  ...
  try {
    // 1. Dynamic Schema Migration: Check if is_synced column exists, add it if not
    const [logColumns] = await conn.query("SHOW COLUMNS FROM activity_logs LIKE 'is_synced'");
    if (logColumns.length === 0) {
      console.log('>>> [Migration] is_synced column is missing in activity_logs. Adding it...');
      ...
      console.log('>>> [Migration] is_synced column successfully created.');
    }
    ...
    console.log('>>> Recalculating and restoring student balances...');
    await conn.beginTransaction();

    for (const st of students) {
      const studentName = st.name;
      const studentNum = Number(st.number);

      // (A) Protect Choi Ah-hyeon (#22) and Oh Ju-won (#13) - keep their current high dividend balances
      if (studentNum === 22 || studentNum === 13) {
        console.log(`[PROTECT] #${studentNum} ${studentName} - Skipped balance calibration.`);
        continue;
      }

      // (B) Hard-fix Hwang Seo-jin (#25) to his correct 540 Cal balance
      if (studentNum === 25 || studentName === '황서진') {
        const targetCal = 540;
        await conn.query('UPDATE students SET calory = ? WHERE id = ?', [targetCal, st.id]);
        console.log(`[RESTORE] #${studentNum} ${studentName} -> Reset to 540 Cal`);
        continue;
      }

      // (C) Match other students by Name from our hardcoded backup database
      const backupSt = BACKUP_STUDENTS[studentName];
      if (!backupSt) {
        console.warn(`[WARN] Student ${studentName} (#${studentNum}) not found in backup file. Skipping.`);
        continue;
      }
      ...
      console.log(`[RESTORE] #${studentNum} ${studentName}: Current=${st.calory} Cal -> Restored=${expectedCalory} Cal (Start=${initialCalory} Cal, Deltas=${sumNewDeltas} Cal)`);
    }

    // (D) Mark all historical logs as is_synced = 1 so they are never double-counted by the server
    await conn.query('UPDATE activity_logs SET is_synced = 1');
    console.log('>>> [Success] All historical activity logs set to is_synced = 1.');

    await conn.commit();
    console.log('\n=============================================');
    console.log('🎉 [COMPLETE] All student balances successfully restored!');
    console.log('=============================================');

  } catch (err) {
    await conn.rollback();
    console.error('❌ [ERROR] Restoration failed and rolled back:', err);
  } finally {
    conn.release();
    process.exit(0);
  }
